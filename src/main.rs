...
mod tests {
    ...
    fn parses_duration_with_fraction() {
        let d = parse_duration("00:01:02.1234567");
        assert_eq!(d.as_secs(), 62);
        assert_eq!(d.subsec_nanos(), 123_456_700);
    }

    #[test]
    fn humanizes_duration() {
        let d = Duration::from_secs(3_723);
        assert_eq!(humanize_duration(d), "1h 2m 3s");
    }

    #[test]
    fn parses_trx_results_and_methods() {
        let xml = r#"<?xml version="1.0" encoding="utf-8"?>
<TestRun>
  <Results>
    <UnitTestResult testId="1" testName="A" outcome="Failed" duration="00:00:01.500">
      <Output>
        <ErrorInfo>
          <Message>boom</Message>
          <StackTrace>at T.M in /repo/test.cs:line 10</StackTrace>
        </ErrorInfo>
      </Output>
    </UnitTestResult>
  </Results>
  <TestDefinitions>
    <UnitTest id="1" name="A">
      <TestMethod className="T" name="M"/>
    </UnitTest>
  </TestDefinitions>
</TestRun>"#;

        let tmp = NamedTempFile::new().expect("temp");
        fs::write(tmp.path(), xml).expect("write");

        let out = parse_trx_file(tmp.path()).expect("parse");
        assert_eq!(out.len(), 1);
        assert_eq!(out[0].method_full_name.as_deref(), Some("T.M"));
        assert_eq!(out[0].outcome, Outcome::Failed);
        assert_eq!(out[0].duration, Duration::from_millis(1500));
    }

    #[test]
    fn run_id_regex_matches() {
        assert!(run_id_matches("abc\n<!-- trx:123 -->\n", "123"));
        assert!(!run_id_matches("<!-- trx:123 -->", "456"));
    }

    #[test]
    fn relative_path_is_calculated() {
        let rel = make_relative(Path::new("/repo"), "/repo/a/b.cs");
        assert_eq!(rel, "a/b.cs");
    }

    #[test]
    fn normalize_file_path_to_parent() {
        let dir = tempfile::tempdir().expect("dir");
        let file = dir.path().join("x.trx");
        fs::write(&file, "").expect("write");
        let normalized = normalize_path(Some(file));
        assert_eq!(normalized, dir.path().to_path_buf());
    }

    #[test]
    fn parse_seconds_handles_no_fraction() {
        let (s, ns) = parse_seconds_fraction("42");
        assert_eq!((s, ns), (42, 0));
    }

    #[test]
    fn compare_names_is_case_insensitive_first() {
        assert_eq!(compare_names("abc", "ABC"), Ordering::Greater);
    }

    #[test]
    fn markdown_indent_preserves_lines() {
        let mut s = String::new();
        append_markdown_indented(&mut s, "a\nb", "> ");
        assert_eq!(s, "> a\n> b\n");
    }

    #[test]
    fn summary_total_is_correct() {
        let summary = Summary {
            passed: 1,
            failed: 2,
            skipped: 3,
    ...
    #[test]
    fn append_badges_adds_expected_labels() {
        let summary = Summary {
            passed: 2,
            failed: 1,
            skipped: 0,
        ...
        append_badges(&summary, &mut out, "10s", None);
        assert!(out.contains("failed-1"));
        assert!(out.contains("passed-2"));
    }

    #[test]
    fn normalize_args_maps_question_help() {
        let args = vec!["trx".to_string(), "-?".to_string()];
        let out = normalize_args(args);
        assert_eq!(out[1], "-h");
    }

    #[test]
    fn push_link_without_url() {
        let mut s = String::new();
        push_link(&mut s, "img", None);
        assert_eq!(s, "img ");
    }

    #[test]
    fn push_link_with_url() {
        let mut s = String::new();
        push_link(&mut s, "img", Some("u"));
        assert_eq!(s, "[img](u) ");
    }

    #[test]
    fn parse_duration_zero_fallback() {
        assert_eq!(parse_duration("x"), Duration::ZERO);
    }
    ...
    #[test]
    fn make_relative_keeps_non_absolute() {
        assert_eq!(make_relative(Path::new("/repo"), "src/a.cs"), "src/a.cs");
    }

    #[test]
    fn run_id_regex_ignores_other_comment() {
        assert!(!run_id_matches("<!-- abc:1 -->", "1"));
    }

    #[test]
    fn summary_humanize_zero_seconds() {
        assert_eq!(humanize_duration(Duration::ZERO), "0s");
    }

    #[test]
    fn format_elapsed_space_encoding() {
        let mut out = String::new();
        let summary = Summary {
            passed: 1,
            failed: 0,
        ...
        append_badges(&summary, &mut out, "1m 5s", None);
        assert!(out.contains("1m%205s"));
    }

    #[test]
    fn normalize_path_for_relative_path() {
        let dir = tempfile::tempdir().expect("dir");
        let old = env::current_dir().expect("cwd");
        env::set_current_dir(dir.path()).expect("cd");
        let p = normalize_path(Some(PathBuf::from("sub")));
        assert!(p.ends_with("sub"));
        env::set_current_dir(old).expect("cd back");
    }

    #[test]
    fn find_child_text_returns_none_for_missing() {
        let doc = Document::parse("<A><B/></A>").expect("doc");
        let root = doc.root_element();
        assert!(find_child_text(root, &["C"]).is_none());
    }

    #[test]
    fn find_descendant_text_works() {
        let doc = Document::parse("<A><B><C>x</C></B></A>").expect("doc");
        let root = doc.root_element();
        assert_eq!(find_descendant_text(root, "C").as_deref(), Some("x"));
    }

    #[test]
    fn is_trx_checks_extension_case_insensitive() {
        assert!(is_trx(Path::new("a.TRX")));
    }

    #[test]
    fn compare_names_tie_breaks() {
        assert_eq!(compare_names("a", "A"), Ordering::Greater);
    }

    #[test]
    fn parse_fraction_truncates_to_nanos() {
        let (_, ns) = parse_seconds_fraction("1.123456789999");
        assert_eq!(ns, 123_456_789);
    }

    #[test]
    fn append_markdown_indented_empty_input() {
        let mut s = String::new();
        append_markdown_indented(&mut s, "", "> ");
        assert_eq!(s, "");
    }

    #[test]
    fn parse_duration_minutes_only() {
        assert_eq!(parse_duration("00:02:00").as_secs(), 120);
    }

    #[test]
    fn parse_duration_invalid_seconds() {
        assert_eq!(parse_duration("00:01:x"), Duration::from_secs(60));
    }

    #[test]
    fn author_contains_version() {
        assert!(author().contains(env!("CARGO_PKG_VERSION")));
    }

    #[test]
    fn os_description_replaces_dash() {
        let d = os_description();
        assert!(!d.contains("--"));
    }

    #[test]
    fn parse_duration_handles_empty() {
        assert_eq!(parse_duration(""), Duration::ZERO);
    }

    #[test]
    fn run_id_regex_needs_digits() {
        assert!(!run_id_matches("<!-- trx:abc -->", "abc"));
    }

    #[test]
    fn parse_duration_parses_hours() {
        assert_eq!(parse_duration("01:00:00").as_secs(), 3600);
    }

    #[test]
    fn append_badges_for_skipped_only() {
        let summary = Summary {
            passed: 0,
            failed: 0,
            skipped: 2,
        ...
        let mut out = String::new();
        append_badges(&summary, &mut out, "1s", Some("http://x"));
        assert!(out.contains("skipped-2"));
        assert!(out.contains("](http://x)"));
    }

    #[test]
    fn parse_seconds_fraction_pads_zeros() {
        let (_, ns) = parse_seconds_fraction("3.5");
        assert_eq!(ns, 500_000_000);
    }

    #[test]
    fn parse_duration_retains_subsec() {
        assert_eq!(parse_duration("00:00:00.001").subsec_millis(), 1);
    }

    #[test]
    fn is_hidden_dir_matches_dot_prefix() {
        assert!(is_hidden_dir(OsStr::new(".git")));
    }

    #[test]
    fn tag_name_for_element() {
        let doc = Document::parse("<X/>").expect("doc");
        assert_eq!(tag_name(doc.root_element()), Some("X"));
    }

    #[test]
    fn tag_name_for_text_node_none() {
        let doc = Document::parse("<X>t</X>").expect("doc");
        let text = doc
            .root_element()
            .children()
            .find(|n| n.is_text())
    ...
    #[test]
    fn make_relative_handles_non_utf8_lossy_path_string() {
        let s: Cow<'_, str> = Path::new("src/a.cs").to_string_lossy();
        assert_eq!(make_relative(Path::new("/repo"), &s), "src/a.cs");
    }
}
